...
class App:
    ...
    def _progress(self, doing):
        ...
        def report(done, total):
            self._post(t("progress_count", doing=doing, done=done, total=total))
        return report
        ...
        try:
            while True:
                ...
                if isinstance(err, core.PrinterSilent):
                    self.status(t("printer_silent_status"), BAD)
                    messagebox.showwarning(
                        t("app_title"),
                        t("printer_silent_body",
                          seconds=int(core.UsbPrinter.timeout
                                      if core.UsbPrinter else 15),
                          step=self._last_step or t("step_starting")))
    ...
    def do_backup(self):
        ...
        report = self._progress(t("progress_reading_memory"))
    ...
    def _reset_work(self, plan):
        ...
        path = self.printer.save_dump(
            tag="pre-reset", progress=self._progress(t("progress_saving_backup")))
        self._post(t("backup_saved_resetting"))
        ok = True
        for addr, value in plan:
            ok = bool(self.printer.ep.write_eeprom((addr, value))) and ok
        self._post(t("reset_written_reading_back"))
